...
public class InputView {
    ...
    public static List<ManualNumbersDto> inputManualNumber(int numberOfManualTicket) {
        System.out.println("수동으로 구매할 번호를 입력해 주세요.");

        List<ManualNumbersDto> manualNumbersDtos = new ArrayList<>();

        for (int i = 0; i < numberOfManualTicket; i++) {
            String[] manualNumbers = SCANNER.nextLine().split(DELIMITER);
            manualNumbersDtos.add(new ManualNumbersDto(manualNumbers));
        }
